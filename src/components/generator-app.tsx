...
type AssetSource = "folder" | "demo";

function SourceLanding({ selecting, error, inputRef, onFiles, onDemo }: {
  selecting: boolean;
  error: string;
  ...
  return <main className="source-landing">
    ...
    <section className="source-choices" aria-label="Choose how to begin">
      <button className="source-card featured" disabled={selecting} onClick={() => inputRef.current?.click()}><span className="source-icon"><FolderOpen /></span><span className="source-badge">Recommended</span><strong>Choose asset folder</strong><small>Select one collection folder containing your layer folders. Files stay on this device.</small><span className="source-action">{selecting ? <><LoaderCircle className="spin" /> Reading artwork…</> : <>Choose folder <ArrowRight size={16} /></>}</span></button>
      <button className="source-card" disabled={selecting} onClick={onDemo}><span className="source-icon demo"><Play /></span><strong>Try the demo</strong><small>Explore all five steps immediately with a small collection generated by the app.</small><span className="source-action">Open demo <ArrowRight size={16} /></span></button>
    </section>
    ...
    <footer className="landing-footer"><span>MIT-licensed software · No account · No backend</span><span>Your artwork stays on your device.</span></footer>
  </main>;
}
...
export function GeneratorApp() {
  ...
  useEffect(() => () => releaseManifest(manifest), [manifest]);
  ...
  if (!manifest || !settings || !source) return <SourceLanding selecting={selecting} error={loadError} inputRef={inputRef} onFiles={(files) => void chooseFiles(files)} onDemo={() => void openDemo()} />;
