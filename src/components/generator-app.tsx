...
import {
  ...
  Eye, FileArchive, FolderDown, ImageIcon, Info, Layers3, LoaderCircle, RefreshCw,
...
const STEP_COPY = [
  ["Your artwork layers", "Every direct folder inside assets becomes one layer."],
...
export function GeneratorApp() {
  ...
  useEffect(() => {
    void fetch("/generated-assets/manifest.json").then(async (response) => {
      ...
      let storedSettings: unknown = defaultSettings(data);
      if (stored) {
        try { storedSettings = JSON.parse(stored); }
        catch { window.localStorage.removeItem(STORAGE_KEY); }
      }
      const next = mergeSettings(data, storedSettings);
      setManifest(data); setSettings(next); setDrafts(structuredClone(next.savedRarities));
    }).catch((reason) => setLoadError(reason instanceof Error ? reason.message : "Could not load assets."));
  }, []);
  ...
  if (loadError) return <main className="fatal"><AlertCircle /><h1>Assets could not be loaded</h1><p>{loadError}</p></main>;
  if (!manifest || !settings) return <main className="splash"><div className="brand-mark"><Sparkles /></div><LoaderCircle className="spin" /><p>Preparing NFT Studio…</p></main>;
  if (!manifest.layers.length) return <main className="empty-project"><div className="brand-mark"><Sparkles /></div><p className="eyebrow">NFT Studio is ready</p><h1>Add your artwork to begin</h1><p>This repository intentionally ships without collection images. Create one direct folder per layer inside <code>assets/</code>, add PNG or WEBP traits, and restart the development server.</p><pre><code>{`assets/\n├── backgrounds/\n│   ├── blue.png\n│   └── red.png\n├── bodies/\n└── hats/`}</code></pre><div className="setup-command"><span>Then run</span><code>npm run dev</code></div><p className="setup-guide">See <strong>README.md</strong> for the complete setup guide.</p></main>;
  return <div className="app-shell"><header className="topbar"><a className="brand" href="#"><span><Sparkles /></span><div><strong>NFT Studio</strong><small>Private, browser-based generator</small></div></a><div className="privacy-pill"><ShieldCheck size={16} /> Your images stay private</div></header><div className="workspace"><Stepper current={step} onSelect={setStep} /><main className="main-panel"><header className="page-heading"><div><p className="eyebrow">Step {step + 1} of 5</p><h1>{subtitle[0]}</h1><p>{subtitle[1]}</p></div><div className="collection-stats"><span><Layers3 size={16} /> {enabledCount} layers</span><span><ImageIcon size={16} /> {manifest.layers.reduce((sum, layer) => sum + layer.traits.length, 0)} traits</span></div></header><section className="step-content">
